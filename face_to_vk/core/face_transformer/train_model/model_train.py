...
class ContrastiveLossHuber(torch.nn.Module):
    def __init__(self, margin=2.0):
        super(ContrastiveLossHuber, self).__init__()
        self.margin = margin
    ...
    def forward(self, output1, output2, label):
        ...
        positive = (1 - label) * torch.abs(euclidean_distance)
        negative = (label) * torch.abs(torch.clamp(self.margin - euclidean_distance, min=0.0))
        huber_pos = torch.where(positive <= 1, positive, torch.pow(positive, 2))
        huber_neg = torch.where(negative <= 1, negative, torch.pow(negative, 2))

        loss_contrastive = torch.mean(huber_pos + huber_neg)
...
class TrainModel:
    ...
    def __init__(
        ...
        self.contrastive_huber_loss = ContrastiveLossHuber(margin=margin)

        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=lr)
        self.lr_scheduler = lr_scheduler
        if lr_scheduler:
            self.scheduler_optimizer = torch.optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer, mode='min', factor=0.75, patience=500)
    ...
    def _contrast_loss(self, func, anchor, positive, negative, backward: bool = True, norm_k: Optional[int] = None):
        loss1 = func(anchor, positive, 0)
        loss2 = func(anchor, negative, 1)
    ...
    def _calc_contrastive_huber_loss(self, anchor, positive, negative, backward: bool = True, norm_k: Optional[int] = None):
        return self._contrast_loss(self.contrastive_huber_loss, anchor, positive, negative, backward, norm_k)

    def _calc_contrastive_loss(self, anchor, positive, negative, backward: bool = True, norm_k: Optional[int] = None):
        return self._contrast_loss(self.contrastive_loss, anchor, positive, negative, backward, norm_k)
    ...
    def _predict_and_calc_loss_on_train(self, batch_size: int, norm_k: Optional[int] = None):
        ...
        elif self.loss_fn_name == 'contrastive_huber':
            loss = self._calc_contrastive_huber_loss(anchor, positive, negative, backward=True, norm_k=norm_k)
        else:
    ...
    def train(self, batch_size: int = 16, epochs: int = 1000, mini_batches: Optional[int] = 1):
        ...
        test_acc, test_auc_roc = None, None
        ...
        for epoch in range(epochs):
            ...
            if not epoch % 10:
                test_acc, test_auc_roc = self._predict_and_calc_score_on_test(
                    ...
                    mini_batches=max(mini_batches // 10, 5))
                if self.writer:
            ...
            print(f"Epoch: {epoch}; "
                  ...
                  (f"Validation metrics: acc - {test_acc:.2f}, auc-roc - {test_auc_roc:.2f}" if test_acc and test_auc_roc
                  else ""))
...
def train(model_name=None, model_class=None, loss_fn=None, lr=None, purge_step=None):
    ...
    train_model = TrainModel(model=cnn_model, train_dir=TRAIN_DIR, test_dir=TEST_DIR, async_mode=True,
                             loss_fn_name=loss_fn_name, lr=lr, lr_scheduler=lr_scheduler, margin=margin)
    train_model.create_tensorboard(
        comment=f" {name}, pretrained {pretrained}, 1 linear layers, no activation,"
                ...
                f" preload local weights {preload_local_weights}, lr {lr}, margin {margin}",
